...
export const UserProvider = ({ children }) => {
 ...
 const emailPasswordSignup = async (email, password) => {
  ...
  const emailPasswordReset = async (email) => {
    try {
      ...
      // Since we are automatically confirming our users, we are going to log in
      // the user using the same credentials once the signup is complete.
      return console.log("Correo enviado validar");
    } catch (error) {
      throw error;
    }
  };
 ...
 return <UserContext.Provider value={{ user, setUser, fetchUser, emailPasswordLogin, emailPasswordSignup, emailPasswordReset, logOutUser }}>
   {children}
